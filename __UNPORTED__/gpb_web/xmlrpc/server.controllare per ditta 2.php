...
cache_time=900' WHERE id='%s';",
	...
	function createproducts($m) {
		...
		$category_id = $sno->scalarval();
		$sno = $m->getParam(3); // tipology_id
		$tipology_id = $sno->scalarval();
		$sno = $m->getParam(4); // currency code (ex. 'EUR')
		$currency_type = $sno->scalarval();

		// Dimension:
		$sno = $m->getParam(5); // product_weight
		$product_weight = $sno->scalarval();
		$sno = $m->getParam(6); // product_length
		$product_length = $sno->scalarval();
		$sno = $m->getParam(7); // product_width
		$product_width = $sno->scalarval();
		$sno = $m->getParam(8); // product_height
	...
	$a=array(
		...
		"openerp.createProducts" => array(  // Only for VM
			...
			"signature" => array(array('int', 'string', 'string', 'int', 'int', 'string', 'double', 'double', 'double', 'double')), 
			"docstring" => 'Create a product. return ID, (code, price, category, typology, Type currency ex. EUR, product_weight, product_length, product_width, product_height)'
		),		
